...
function renderHome() {
    let productsList = (window.products || JSON.parse(localStorage.getItem('products')) || []).slice();
    let donations = JSON.parse(localStorage.getItem('donations')) || [];
    let donatedNames = new Set();
    donations.forEach(donation => {
        if (!donatedNames.has(donation.productName)) {
            productsList.push({
                name: donation.productName,
                price: donation.productPrice,
                image: donation.productPic,
                description: 'Sold by: ' + donation.username,
                available: true,
                rating: '',
                category: donation.category || 'Other',
            });
            donatedNames.add(donation.productName);
        }
    });
    // Group products by category, changing 'Produce' to 'Fruits'
    const categories = {};
    productsList.forEach(product => {
        let cat = (product.category || 'Other');
        if (cat.toLowerCase() === 'produce') cat = 'Fruits';
        if (!categories[cat]) categories[cat] = [];
        categories[cat].push(product);
    });
    let html = `<div class='pb-16'>`;
    Object.keys(categories).forEach(cat => {
        html += `<h2 class='text-xl font-bold mb-4 mt-6'>${cat}</h2>`;
        html += `<div class="grid grid-cols-2 gap-4 px-4">`;
        categories[cat].forEach(product => {
            html += `<div class="product-card bg-white rounded-lg shadow-md overflow-hidden">
                <div class="relative">
...
function cancelOrder(orderId) {
    ...
    viewAllOrders(); // Refresh the order list
}
...
function renderAIAssistant() {
    ...
    assistantContainer.innerHTML = `
        <div id="ai-chat-button" class="fixed bottom-20 right-5 bg-green-600 w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg cursor-pointer z-40">
            <i class="fas fa-robot text-2xl"></i>
        </div>
        <div id="ai-chat-window" class="hidden fixed bottom-5 right-5 w-80 h-96 bg-white rounded-lg shadow-2xl flex flex-col z-50">
            <div class="bg-green-600 text-white p-3 rounded-t-lg flex justify-between items-center">
                <h3 class="font-bold">SaveMart Assistant</h3>
                <button id="ai-close-button" class="text-white">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div id="ai-chat-messages" class="flex-1 p-4 overflow-y-auto">
                <div class="flex mb-3">
                    <div class="bg-gray-200 p-3 rounded-lg">
                        <p class="text-sm">Hello! How can I help you find the freshest groceries today?</p>
                    </div>
                </div>
            </div>
            <div class="p-3 border-t">
                <div class="flex">
                    <input type="text" id="ai-chat-input" class="w-full px-3 py-2 border rounded-l-md focus:outline-none" placeholder="Ask about products...">
                    <button id="ai-send-button" class="bg-green-600 text-white px-4 rounded-r-md">Send</button>
                </div>
            </div>
        </div>
    `;
    ...
    document.getElementById('ai-close-button').addEventListener('click', () => {
        document.getElementById('ai-chat-window').classList.add('hidden');
    });

    document.getElementById('ai-send-button').addEventListener('click', handleAIChat);
...
function addMessageToChat(message, sender) {
    ...
    messageDiv.innerHTML = `
        <div class="${sender === 'user' ? 'bg-green-500 text-white' : 'bg-gray-200'} p-3 rounded-lg max-w-xs">
...
function processAIQuery(query) {
    ...
    let response = "I'm sorry, I couldn't find information about that. You can ask me about product prices, descriptions, or availability.";

    const allProducts = JSON.parse(localStorage.getItem('products')) || [];

    // Check for greeting
    if (query.includes('hello') || query.includes('hi')) {
        response = "Hello there! How can I help you with your shopping today?";
    }
    // Check for price
    else if (query.includes('price of') || query.includes('how much')) {
        const productName = query.replace('price of', '').replace('how much is', '').replace('the','').trim();
        const product = allProducts.find(p => p.name.toLowerCase().includes(productName));
        if (product) {
            response = `The price of ${product.name} is $${product.price.toFixed(2)} ${product.unit ? `(${product.unit})` : ''}.`;
        }
    }
    // Check for description
    else if (query.includes('tell me about') || query.includes('what is')) {
        const productName = query.replace('tell me about', '').replace('what is', '').replace('the','').trim();
        const product = allProducts.find(p => p.name.toLowerCase().includes(productName));
        if (product) {
    ...
    else if (query.includes('do you have') || query.includes('in stock')) {
        const productName = query.replace('do you have', '').replace('are in stock','').trim();
        const product = allProducts.find(p => p.name.toLowerCase().includes(productName));
        if (product) {
            response = product.stock > 0 ? `Yes, ${product.name} is in stock. We have ${product.stock} available.` : `Sorry, ${product.name} is currently out of stock.`;
        }
    }
    // General search
    else {
        const products = allProducts.filter(p => p.name.toLowerCase().includes(query) || p.category.toLowerCase().includes(query));
        if (products.length > 0) {
            response = `I found these products for you: ${products.map(p => p.name).join(', ')}.`;
        }
    }
...
document.addEventListener('DOMContentLoaded', renderAIAssistant);

// ZeroWasteMart navigation support
function navigateZeroWasteMart() {
    document.getElementById('app').style.display = 'none';
    document.getElementById('zerowaste-section').style.display = 'block';
    renderZeroWasteMart();
}

function renderZeroWasteMart() {
    let listings = JSON.parse(localStorage.getItem('zerowaste_listings')) || [];
    let html = `<h2 class='text-2xl font-bold mb-4'>ZeroWasteMart</h2>`;
    html += `<button id='add-zerowaste-listing' class='bg-emerald-500 text-white px-4 py-2 rounded mb-4'>Add Listing</button>`;
    if (listings.length === 0) {
        html += `<p>No listings yet. Be the first to add!</p>`;
    } else {
        ...
        listings.forEach((item, idx) => {
            html += `<div class='bg-white p-4 rounded shadow'>
                ...
                <p class='text-sm text-gray-500'>${item.price ? 'Price: ₹' + item.price : 'Free'}</p>
                <p class='text-sm text-gray-500'>Contact: ${item.contact}</p>
                <button onclick='markAsGone(${idx})' class='mt-2 bg-gray-200 px-2 py-1 rounded'>Mark as Gone</button>
            </div>`;
        });
        html += `</div>`;
    }
    document.getElementById('zerowaste-section').innerHTML = html;
    document.getElementById('add-zerowaste-listing').onclick = showZeroWasteForm;
}

function showZeroWasteForm() {
    let formHtml = `<div class='bg-white p-4 rounded shadow mb-4'>
        <h3 class='font-bold mb-2'>Add ZeroWasteMart Listing</h3>
        <input id='zw-name' class='border p-2 mb-2 w-full' placeholder='Item Name'><br>
        <textarea id='zw-desc' class='border p-2 mb-2 w-full' placeholder='Description'></textarea><br>
        <input id='zw-category' class='border p-2 mb-2 w-full' placeholder='Category (e.g. Groceries, Household)'><br>
        <input id='zw-condition' class='border p-2 mb-2 w-full' placeholder='Condition (e.g. New, Used)'><br>
        <input id='zw-price' class='border p-2 mb-2 w-full' placeholder='Price (leave blank for Free)'><br>
        <input id='zw-contact' class='border p-2 mb-2 w-full' placeholder='Contact Info'><br>
        <button id='save-zw-listing' class='bg-emerald-500 text-white px-4 py-2 rounded'>Save</button>
        <button id='cancel-zw-listing' class='ml-2 bg-gray-300 px-4 py-2 rounded'>Cancel</button>
    </div>`;
    document.getElementById('zerowaste-section').innerHTML = formHtml;
    document.getElementById('save-zw-listing').onclick = saveZeroWasteListing;
    document.getElementById('cancel-zw-listing').onclick = renderZeroWasteMart;
}

function saveZeroWasteListing() {
    let listings = JSON.parse(localStorage.getItem('zerowaste_listings')) || [];
    let item = {
        name: document.getElementById('zw-name').value,
        description: document.getElementById('zw-desc').value,
        category: document.getElementById('zw-category').value,
        condition: document.getElementById('zw-condition').value,
        price: document.getElementById('zw-price').value,
        contact: document.getElementById('zw-contact').value
    };
    listings.push(item);
    localStorage.setItem('zerowaste_listings', JSON.stringify(listings));
    renderZeroWasteMart();
}

function markAsGone(idx) {
    let listings = JSON.parse(localStorage.getItem('zerowaste_listings')) || [];
